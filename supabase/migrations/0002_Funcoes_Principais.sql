-- supabase/migrations/0002_Funcoes_Principais.sql
-- =============================================================================
...
-- Função para apagar endereço (versão segura)
...
-- Função para buscar clientes por endereço
...
-- CORREÇÃO: Removido o prefixo 'p_' dos parâmetros para corresponder à chamada do JavaScript.
...
BEGIN
    ...
    IF address_id IS NULL THEN
        ...
        VALUES (v_cleaned_cep, street_name, neighborhood, city_id)
...
BEGIN
    ...
    FROM public.cities c JOIN public.states s ON c.state_id = s.id
    ...
    WHERE a.cep = v_cleaned_cep AND f_unaccent(a.street_name) ILIKE f_unaccent(p_street_name) AND a.city_id = v_city_id
    LIMIT 1;
    RETURN result_address;
END;
$$;
...
BEGIN
    ...
    ELSE
        UPDATE public.customers SET
            full_name = p_full_name, cpf = p_cpf, cellphone = p_cellphone, email = p_email, birth_date = p_birth_date,
            contact_customer_id = p_contact_customer_id, address_id = p_address_id, address_number = p_address_number,
...
BEGIN
    ...
    WHERE (p_search_term IS NULL OR p_search_term = '' OR (f_unaccent(full_name) ILIKE f_unaccent('%' || p_search_term || '%') OR cpf ILIKE '%' || p_search_term || '%'))
    AND (p_status_filter IS NULL OR p_status_filter = 'all' OR (p_status_filter = 'active' AND is_active = TRUE) OR (p_status_filter = 'inactive' AND is_active = FALSE));
    RETURN total_count;
END;
$$;
...
BEGIN
    ...
    WITH filtered_customers AS (
        ...
        WHERE (p_search_term IS NULL OR p_search_term = '' OR (f_unaccent(c.full_name) ILIKE f_unaccent('%' || p_search_term || '%') OR c.cpf ILIKE '%' || p_search_term || '%'))
        AND (p_status_filter IS NULL OR p_status_filter = 'all' OR (p_status_filter = 'active' AND c.is_active = TRUE) OR (p_status_filter = 'inactive' AND c.is_active = FALSE))
    )
    SELECT fc.id, fc.full_name::text, fc.cpf, fc.cellphone, fc.is_active,
...
BEGIN
    ...
    FROM public.customers c LEFT JOIN public.addresses a ON c.address_id = a.id
...
BEGIN
    ...
    UPDATE public.objects SET is_archived = TRUE WHERE status IN ('Entregue', 'Devolvido') AND is_archived = FALSE AND updated_at < (NOW() - INTERVAL '30 days');
END;
$$;
