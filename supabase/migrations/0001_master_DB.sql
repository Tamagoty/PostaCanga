...
CREATE OR REPLACE FUNCTION revert_object_status(p_control_number INT) RETURNS void LANGUAGE plpgsql SECURITY DEFINER AS $$ BEGIN UPDATE public.objects SET status = 'Aguardando Retirada' WHERE control_number = p_control_number; END; $$;
DROP FUNCTION IF EXISTS public.count_addresses(integer, text, text);
CREATE OR REPLACE FUNCTION count_addresses(p_city_id INT, p_neighborhood TEXT, p_search_term TEXT) RETURNS INT LANGUAGE plpgsql AS $$ DECLARE total_count INT; BEGIN SELECT COUNT(*) INTO total_count FROM public.addresses WHERE (p_city_id IS NULL OR city_id = p_city_id) AND (p_neighborhood IS NULL OR public.addresses.neighborhood ILIKE p_neighborhood) AND (p_search_term IS NULL OR (f_unaccent(street_name) ILIKE f_unaccent('%' || p_search_term || '%') OR cep ILIKE '%' || p_search_term || '%')); RETURN total_count; END; $$;
DROP FUNCTION IF EXISTS public.get_neighborhoods_by_city(integer);
CREATE OR REPLACE FUNCTION get_neighborhoods_by_city(p_city_id INT) RETURNS TABLE(neighborhood TEXT) LANGUAGE sql AS $$ SELECT DISTINCT neighborhood FROM public.addresses WHERE city_id = p_city_id AND neighborhood IS NOT NULL ORDER BY neighborhood; $$;
DROP FUNCTION IF EXISTS public.search_cities(text);
CREATE OR REPLACE FUNCTION search_cities(p_search_term TEXT) RETURNS TABLE(id INT, name VARCHAR, uf CHAR(2)) LANGUAGE plpgsql AS $$ BEGIN RETURN QUERY SELECT c.id, c.name, s.uf FROM public.cities c JOIN public.states s ON c.state_id = s.id WHERE f_unaccent(c.name) ILIKE f_unaccent('%' || p_search_term || '%') ORDER BY c.name LIMIT 20; END; $$;
DROP FUNCTION IF EXISTS public.count_links(text);
CREATE OR REPLACE FUNCTION count_links(p_search_term TEXT) RETURNS INT LANGUAGE plpgsql AS $$ DECLARE total_count INT; BEGIN SELECT COUNT(*) INTO total_count FROM public.system_links WHERE p_search_term IS NULL OR f_unaccent(name) ILIKE f_unaccent('%' || p_search_term || '%') OR f_unaccent(description) ILIKE f_unaccent('%' || p_search_term || '%'); RETURN total_count; END; $$;
CREATE OR REPLACE FUNCTION get_message_templates() RETURNS SETOF message_templates LANGUAGE sql STABLE AS $$ SELECT * FROM public.message_templates ORDER BY name; $$;
CREATE OR REPLACE FUNCTION create_or_update_message_template(p_id UUID, p_name TEXT, p_content TEXT) RETURNS message_templates LANGUAGE plpgsql SECURITY DEFINER AS $$ DECLARE result_template message_templates; BEGIN IF p_id IS NULL THEN INSERT INTO public.message_templates (name, content) VALUES (p_name, p_content) RETURNING * INTO result_template; ELSE UPDATE public.message_templates SET name = p_name, content = p_content, updated_at = NOW() WHERE id = p_id RETURNING * INTO result_template; END IF; RETURN result_template; END; $$;
CREATE OR REPLACE FUNCTION delete_message_template(p_id UUID) RETURNS void LANGUAGE plpgsql SECURITY DEFINER AS $$ BEGIN DELETE FROM public.message_templates WHERE id = p_id; END; $$;
...
-- [CORREÇÃO] Apaga todas as políticas antes de recriar a função is_admin
...
-- Agora é seguro apagar e recriar a função is_admin
...
-- Recria todas as políticas
DROP POLICY IF EXISTS "Allow read access to all authenticated users" ON public.states; CREATE POLICY "Allow read access to all authenticated users" ON public.states FOR SELECT TO authenticated USING (true);
DROP POLICY IF EXISTS "Allow read access to all authenticated users" ON public.cities; CREATE POLICY "Allow read access to all authenticated users" ON public.cities FOR SELECT TO authenticated USING (true);
DROP POLICY IF EXISTS "Allow read access to object types" ON public.object_types; CREATE POLICY "Allow read access to object types" ON public.object_types FOR SELECT TO authenticated USING (true);
DROP POLICY IF EXISTS "Allow read access to tracking rules" ON public.tracking_code_rules; CREATE POLICY "Allow read access to tracking rules" ON public.tracking_code_rules FOR SELECT TO authenticated USING (true);
DROP POLICY IF EXISTS "Allow read access to app settings" ON public.app_settings; CREATE POLICY "Allow read access to app settings" ON public.app_settings FOR SELECT TO authenticated USING (true);
DROP POLICY IF EXISTS "Employees can manage data" ON public.addresses; CREATE POLICY "Employees can manage data" ON public.addresses FOR ALL TO authenticated USING (true) WITH CHECK (true);
DROP POLICY IF EXISTS "Employees can manage data" ON public.customers; CREATE POLICY "Employees can manage data" ON public.customers FOR ALL TO authenticated USING (true) WITH CHECK (true);
DROP POLICY IF EXISTS "Employees can manage data" ON public.objects; CREATE POLICY "Employees can manage data" ON public.objects FOR ALL TO authenticated USING (true) WITH CHECK (true);
DROP POLICY IF EXISTS "Employees can manage data" ON public.office_supplies; CREATE POLICY "Employees can manage data" ON public.office_supplies FOR ALL TO authenticated USING (true) WITH CHECK (true);
DROP POLICY IF EXISTS "Employees can manage data" ON public.supply_stock_log; CREATE POLICY "Employees can manage data" ON public.supply_stock_log FOR ALL TO authenticated USING (true) WITH CHECK (true);
DROP POLICY IF EXISTS "Employees can manage data" ON public.bulk_import_reports; CREATE POLICY "Employees can manage data" ON public.bulk_import_reports FOR ALL TO authenticated USING (true) WITH CHECK (true);
DROP POLICY IF EXISTS "Employees can manage data" ON public.system_links; CREATE POLICY "Employees can manage data" ON public.system_links FOR ALL TO authenticated USING (true) WITH CHECK (true);
DROP POLICY IF EXISTS "Users can manage their own themes" ON public.user_themes; CREATE POLICY "Users can manage their own themes" ON public.user_themes FOR ALL TO authenticated USING (user_id = auth.uid()) WITH CHECK (user_id = auth.uid());
DROP POLICY IF EXISTS "Employees can view their own data" ON public.employees; CREATE POLICY "Employees can view their own data" ON public.employees FOR SELECT TO authenticated USING (auth.uid() = id);
...
DROP POLICY IF EXISTS "Authenticated users can manage message templates" ON public.message_templates; CREATE POLICY "Authenticated users can manage message templates" ON public.message_templates FOR ALL TO authenticated USING (true) WITH CHECK (true);
