-- Arquivo: supabase/migrations/000_consolidated_schema.sql
-- Descrição: Script único para inicializar todo o banco de dados do PostaCanga do zero.
-- Versão: 1.0
-- DATA DE CONSOLIDAÇÃO: 2024-07-18
...
CREATE TABLE IF NOT EXISTS public.addresses (
    ...
    UNIQUE(street_name, city_id, cep)
);
...
-- Tabela de Regras de Rastreamento
...
-- Tabela de Relatórios de Inserção em Massa
...
--------------------------------------------------------------------------------
-- DADOS INICIAIS (SEMENTES)
--------------------------------------------------------------------------------
INSERT INTO public.object_types (name, default_storage_days) VALUES
('Encomenda PAC', 7), ('SEDEX', 7), ('Carta Registrada', 20),
('Carta Simples', 20), ('Revista', 20), ('Cartão', 20),
('Telegrama', 7), ('Cartão Registrado', 20), ('Registrado', 7), ('Outro', 7)
ON CONFLICT (name) DO NOTHING;

INSERT INTO public.tracking_code_rules (prefix, object_type, storage_days) VALUES
('AC', 'Encomenda PAC', 7), ('QB', 'Encomenda PAC', 7), ('SS', 'SEDEX', 7),
('BR', 'Carta Registrada', 20), ('YD', 'Cartão Registrado', 20)
ON CONFLICT (prefix) DO NOTHING;

INSERT INTO public.app_settings (key, value, description) VALUES
('agency_name', 'Correio de América Dourada', 'Nome da agência exibido no sistema.'),
...
--------------------------------------------------------------------------------
-- TIPOS CUSTOMIZADOS (PARA FUNÇÕES)
--------------------------------------------------------------------------------
DO $$ BEGIN
...
CREATE OR REPLACE FUNCTION proper_case(p_text TEXT) RETURNS TEXT AS $$
BEGIN
...
CREATE OR REPLACE FUNCTION delete_address(p_address_id UUID) RETURNS VOID LANGUAGE plpgsql SECURITY DEFINER AS $$
BEGIN
    IF EXISTS (SELECT 1 FROM public.customers WHERE address_id = p_address_id) THEN
        RAISE EXCEPTION 'Endereço em uso por um cliente.';
    END IF;
...
BEGIN
    RETURN QUERY SELECT c.id, c.full_name FROM public.customers c
    WHERE c.is_active = TRUE AND c.cellphone IS NOT NULL AND normalize_text(c.full_name) ILIKE normalize_text('%' || p_search_term || '%')
...
DECLARE v_customer_id UUID; v_address_id UUID; v_city_id INT; v_storage_deadline DATE; v_storage_days INT; result_object objects;
BEGIN
    ...
    IF p_street_name IS NOT NULL AND p_city_name IS NOT NULL THEN
        ...
        IF v_city_id IS NULL THEN RAISE EXCEPTION 'Cidade ou Estado não encontrado: %, %', p_city_name, p_state_uf; END IF;
        INSERT INTO public.addresses (cep, street_name, neighborhood, city_id) VALUES (p_cep, p_street_name, p_neighborhood, v_city_id) ON CONFLICT DO NOTHING;
        SELECT id INTO v_address_id FROM public.addresses WHERE street_name=p_street_name AND city_id=v_city_id;
    ...
    ELSE
        INSERT INTO public.objects (recipient_name, object_type, storage_deadline, tracking_code, customer_id, delivery_address_id) VALUES (p_recipient_name, p_object_type, v_storage_deadline, p_tracking_code, v_customer_id, v_address_id) RETURNING * INTO result_object;
    END IF;
...
BEGIN IF get_my_role() <> 'admin' THEN RAISE EXCEPTION 'Apenas administradores podem executar esta ação.'; END IF; UPDATE public.objects SET is_archived = TRUE WHERE status IN ('Entregue', 'Devolvido') AND is_archived = FALSE; END; $$;
CREATE OR REPLACE FUNCTION unarchive_object(p_control_number INT) RETURNS VOID LANGUAGE plpgsql SECURITY DEFINER AS $$
BEGIN IF get_my_role() <> 'admin' THEN RAISE EXCEPTION 'Apenas administradores podem executar esta ação.'; END IF; UPDATE public.objects SET status = 'Aguardando Retirada', is_archived = FALSE WHERE control_number = p_control_number; END; $$;
...
RETURNS TABLE (report_recipient_name TEXT, report_control_number INT) LANGUAGE plpgsql SECURITY DEFINER AS $$
DECLARE obj simple_object_input; v_recipient_name TEXT; v_street_name TEXT; v_address_id UUID; v_city_id INT; v_customer_id UUID; v_storage_deadline DATE; v_new_control_number INT; v_storage_days INT;
BEGIN
    ...
    FOREACH obj IN ARRAY p_objects LOOP
        ...
        IF v_address_id IS NULL THEN INSERT INTO addresses (street_name, city_id) VALUES (v_street_name, v_city_id) RETURNING id INTO v_address_id; END IF;
        SELECT id INTO v_customer_id FROM customers WHERE normalize_text(full_name) = normalize_text(v_recipient_name) LIMIT 1;
        INSERT INTO public.objects (recipient_name, object_type, storage_deadline, customer_id, delivery_address_id) VALUES (v_recipient_name, p_object_type, v_storage_deadline, v_customer_id, v_address_id) RETURNING control_number INTO v_new_control_number;
        report_recipient_name := v_recipient_name; report_control_number := v_new_control_number;
        RETURN NEXT;
    END LOOP;
    RETURN;
END;
...
RETURNS TABLE (report_recipient_name TEXT, report_control_number INT, report_tracking_code TEXT) LANGUAGE plpgsql SECURITY DEFINER AS $$
DECLARE obj registered_object_input; v_rule RECORD; v_recipient_name TEXT; v_street_name TEXT; v_address_number TEXT; v_address_complement TEXT; v_address_id UUID; v_city_id INT; v_customer_id UUID; v_storage_deadline DATE; v_new_control_number INT;
BEGIN
    SELECT id INTO v_city_id FROM cities WHERE name = 'América Dourada' LIMIT 1;
    IF v_city_id IS NULL THEN RAISE EXCEPTION 'A cidade padrão "América Dourada" não foi encontrada.'; END IF;
    FOREACH obj IN ARRAY p_objects LOOP
        ...
        END IF;
        v_recipient_name := proper_case(obj.recipient_name); v_street_name := proper_case(obj.street_name); v_address_number := obj.address_number; v_address_complement := obj.address_complement;
        SELECT id INTO v_address_id FROM addresses WHERE normalize_text(street_name) = normalize_text(v_street_name) AND city_id = v_city_id LIMIT 1;
        IF v_address_id IS NULL THEN INSERT INTO addresses (street_name, city_id) VALUES (v_street_name, v_city_id) RETURNING id INTO v_address_id; END IF;
        SELECT id INTO v_customer_id FROM customers WHERE normalize_text(full_name) = normalize_text(v_recipient_name) LIMIT 1;
        IF v_customer_id IS NULL THEN INSERT INTO public.customers (full_name, address_id, address_number, address_complement) VALUES (v_recipient_name, v_address_id, v_address_number, v_address_complement) RETURNING id INTO v_customer_id;
        ELSE UPDATE public.customers SET address_id = v_address_id, address_number = v_address_number, address_complement = v_address_complement WHERE id = v_customer_id;
        END IF;
        INSERT INTO public.objects (recipient_name, object_type, tracking_code, storage_deadline, customer_id, delivery_address_id) VALUES (v_recipient_name, obj.object_type, upper(obj.tracking_code), v_storage_deadline, v_customer_id, v_address_id) RETURNING control_number INTO v_new_control_number;
        report_recipient_name := v_recipient_name; report_control_number := v_new_control_number; report_tracking_code := upper(obj.tracking_code);
        RETURN NEXT;
    END LOOP;
    RETURN;
END;
...
CREATE OR REPLACE FUNCTION create_or_update_app_setting(p_key TEXT, p_value TEXT, p_description TEXT)
RETURNS VOID LANGUAGE plpgsql SECURITY DEFINER AS $$
BEGIN
    IF get_my_role() <> 'admin' THEN RAISE EXCEPTION 'Apenas administradores podem executar esta ação.'; END IF;
    INSERT INTO public.app_settings (key, value, description) VALUES (p_key, p_value, p_description)
...
RETURNS TABLE (id UUID, full_name TEXT, registration_number VARCHAR, role TEXT) LANGUAGE plpgsql SECURITY DEFINER AS $$
BEGIN
    IF get_my_role() <> 'admin' THEN RAISE EXCEPTION 'Apenas administradores podem visualizar perfis.'; END IF;
    RETURN QUERY SELECT e.id, e.full_name, e.registration_number, e.role FROM public.employees e;
END;
$$;

CREATE OR REPLACE FUNCTION delete_employee(p_user_id UUID) RETURNS VOID LANGUAGE plpgsql SECURITY DEFINER AS $$
BEGIN
    IF get_my_role() <> 'admin' THEN RAISE EXCEPTION 'Apenas administradores podem apagar usuários.'; END IF;
...
BEGIN
    IF get_my_role() <> 'admin' THEN RAISE EXCEPTION 'Apenas administradores podem executar esta ação.'; END IF;
    IF p_rule_id IS NOT NULL THEN UPDATE public.tracking_code_rules SET prefix=p_prefix, object_type=p_object_type, storage_days=p_storage_days WHERE id = p_rule_id;
    ELSE INSERT INTO public.tracking_code_rules (prefix, object_type, storage_days) VALUES (p_prefix, p_object_type, p_storage_days);
    END IF;
END;
$$;
...
BEGIN
    IF get_my_role() <> 'admin' THEN RAISE EXCEPTION 'Apenas administradores podem executar esta ação.'; END IF;
    IF p_type_id IS NOT NULL THEN UPDATE public.object_types SET name=p_name, default_storage_days=p_default_storage_days WHERE id = p_type_id;
    ELSE INSERT INTO public.object_types (name, default_storage_days) VALUES (p_name, p_default_storage_days);
    END IF;
END;
$$;
...
CREATE OR REPLACE FUNCTION create_or_update_task(p_task_id INT, p_title TEXT, p_description TEXT, p_frequency_type TEXT, p_due_date DATE DEFAULT NULL)
RETURNS VOID LANGUAGE plpgsql SECURITY DEFINER AS $$
BEGIN
    IF get_my_role() <> 'admin' THEN RAISE EXCEPTION 'Apenas administradores podem executar esta ação.'; END IF;
    IF p_task_id IS NOT NULL THEN UPDATE public.tasks SET title = p_title, description = p_description, frequency_type = p_frequency_type, due_date = p_due_date WHERE id = p_task_id;
    ELSE INSERT INTO public.tasks (title, description, frequency_type, due_date) VALUES (p_title, p_description, p_frequency_type, p_due_date);
    END IF;
END;
$$;
...
RETURNS TABLE (id INT, title TEXT, description TEXT, frequency_type TEXT, due_date DATE, last_completed_at TIMESTAMPTZ) LANGUAGE plpgsql AS $$
BEGIN
    RETURN QUERY
    SELECT t.id, t.title, t.description, t.frequency_type, t.due_date, (SELECT MAX(tc.completed_at) FROM task_completions tc WHERE tc.task_id = t.id) AS last_completed_at
    FROM public.tasks t
    WHERE t.is_active = TRUE AND (
        ...
        (t.frequency_type = 'monthly' AND ((SELECT MAX(completed_at) FROM task_completions WHERE task_id = t.id) IS NULL OR (EXTRACT(YEAR FROM (SELECT MAX(completed_at) FROM task_completions WHERE task_id = t.id)) * 12 + EXTRACT(MONTH FROM (SELECT MAX(completed_at) FROM task_completions WHERE task_id = t.id))) < (EXTRACT(YEAR FROM CURRENT_DATE) * 12 + EXTRACT(MONTH FROM CURRENT_DATE)))) OR
        (t.frequency_type = 'quarterly' AND ((SELECT MAX(completed_at) FROM task_completions WHERE task_id = t.id) IS NULL OR (SELECT MAX(completed_at) FROM task_completions WHERE task_id = t.id) < NOW() - INTERVAL '3 months')) OR
        (t.frequency_type = 'semiannual' AND ((SELECT MAX(completed_at) FROM task_completions WHERE task_id = t.id) IS NULL OR (SELECT MAX(completed_at) FROM task_completions WHERE task_id = t.id) < NOW() - INTERVAL '6 months')) OR
        (t.frequency_type = 'annual' AND ((SELECT MAX(completed_at) FROM task_completions WHERE task_id = t.id) IS NULL OR (SELECT MAX(completed_at) FROM task_completions WHERE task_id = t.id) < NOW() - INTERVAL '1 year')) OR
...
RETURNS VOID LANGUAGE plpgsql SECURITY DEFINER AS $$
BEGIN INSERT INTO public.task_completions (task_id, user_id) VALUES (p_task_id, auth.uid()); END;
$$;
...
BEGIN
    ...
    SELECT json_agg(b) INTO v_upcoming_birthdays FROM (SELECT id, full_name, birth_date FROM public.customers WHERE is_active = TRUE AND birth_date IS NOT NULL) b;
...
RETURNS TABLE (id BIGINT, quantity_changed INT, new_stock_total INT, reason TEXT, created_at TIMESTAMPTZ, user_id UUID) LANGUAGE plpgsql SECURITY DEFINER AS $$
BEGIN
    RETURN QUERY SELECT l.id, l.quantity_changed, l.new_stock_total, l.reason, l.created_at, l.user_id FROM public.supply_stock_log l WHERE l.supply_id = p_supply_id AND l.created_at >= p_start_date ORDER BY l.created_at DESC;
END;
$$;
...
CREATE OR REPLACE FUNCTION save_bulk_report(p_report_data JSONB)
RETURNS VOID LANGUAGE plpgsql SECURITY DEFINER AS $$
BEGIN INSERT INTO public.bulk_import_reports (user_id, report_data) VALUES (auth.uid(), p_report_data); END;
$$;
...
-- Apagar políticas antigas para evitar duplicatas
...
DROP POLICY IF EXISTS "Employees can manage addresses" ON public.addresses;
DROP POLICY IF EXISTS "Employees can manage customers" ON public.customers;
DROP POLICY IF EXISTS "Employees can manage objects" ON public.objects;
DROP POLICY IF EXISTS "Employees can manage office supplies" ON public.office_supplies;
DROP POLICY IF EXISTS "Employees can manage stock log" ON public.supply_stock_log;
...
DROP POLICY IF EXISTS "Allow read access to all authenticated users" ON public.object_types;
...
-- (Adicione drops para outras tabelas se necessário)
