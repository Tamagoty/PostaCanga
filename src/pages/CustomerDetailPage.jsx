...
const CustomerDetailPage = () => {
  ...
  const fetchDetails = useCallback(async () => {
    ...
    if (error) {
      toast.error('Erro ao buscar detalhes do cliente: ' + error.message);
  ...
  if (loading) {
    return <div className={styles.loading}>A carregar detalhes...</div>;
  }

  if (!customerDetails?.profile) {
    return <div className={styles.loading}>Cliente não encontrado.</div>;
  }
    ...
    <div className={styles.container}>
      ...
      <div className={styles.profileGrid}>
        <div className={styles.profileCard}>
          <FaUser className={styles.icon} /> <h3>{profile.full_name}</h3>
        </div>
        <div className={styles.profileCard}>
          <FaPhone className={styles.icon} /> <p>{profile.cellphone || 'Não informado'}</p>
        </div>
        <div className={styles.profileCard}>
          <FaIdCard className={styles.icon} /> <p>{profile.cpf || 'Não informado'}</p>
        </div>
        <div className={styles.profileCard}>
          <FaBirthdayCake className={styles.icon} /> <p>{profile.birth_date ? new Date(profile.birth_date).toLocaleDateString('pt-BR', {timeZone: 'UTC'}) : 'Não informado'}</p>
        </div>
        <div className={`${styles.profileCard} ${styles.addressCard}`}>
          <FaMapMarkerAlt className={styles.icon} />
          <p>{profile.addresses ? `${profile.addresses.street_name}, ${profile.addresses.city} - ${profile.addresses.state}` : 'Endereço não informado'}</p>
        </div>
      </div>
              ...
              {objects.length > 0 ? (
                objects.map(obj => (
                  <tr key={obj.control_number}>
                    ...
                    <td data-label="Prazo de Guarda">
                      <ProgressBar startDate={obj.arrival_date} endDate={obj.storage_deadline} status={obj.status} />
                    </td>
